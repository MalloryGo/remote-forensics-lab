...
curl -fsSL "$RAW_BASE/assets/performance_0420.xlsx" -o /tmp/remote-forensics-assets/performance_0420.xlsx
...
scp -q -o StrictHostKeyChecking=no /tmp/remote-forensics-assets/performance_0420.xlsx "$TARGET_HOST:/tmp/performance_0420.xlsx"
...
install -m 644 /tmp/performance_0420.xlsx "$BASE/assets/4.20 (3).xlsx"
...
<!doctype html><html><head><meta charset="utf-8"><title>WH838 Gateway</title></head>
...
if command -v crictl >/dev/null 2>&1; then
  for name in kube-apiserver kube-controller-manager kube-scheduler etcd; do
    ids="$(crictl ps --name "$name" -q 2>/dev/null || true)"; [ -z "$ids" ] || crictl stop $ids >/dev/null 2>&1 || true
  done
fi
